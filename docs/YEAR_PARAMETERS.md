...
|----------|----------------|---------|
| `MIN_HISTORICAL_YEAR` | `2020` | Earliest season to load (explicit input constant). |
...
| `SEASONS_TO_SCRAPE` | `range(MIN_HISTORICAL_YEAR, CURRENT_NFL_SEASON + 1)` | Default load range. |
...
| **schedule loader** (CLI `--year`) | `year` | `get_current_nfl_season()` (nfl_calendar). |
