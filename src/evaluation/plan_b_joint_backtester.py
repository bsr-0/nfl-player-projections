"""Seasonal, identical-row comparison for the Plan B grouped allocation arm."""
...
ARMS = ("rolling3", "normalized_prior", "joint_other")
...
def run_backtest(panel: pd.DataFrame, target: str, seasons: list[int], *,
                 epsilon: float = 0.001, penalty: float = 0.001,
    ...
    if total not in panel:
        raise ValueError(f"missing training team-total label {total}")
    ...
    grouped = panel.groupby(GROUP)[label].sum()
    if (grouped > 1 + 1e-8).any():
    ...
    for fold in meta["folds"]:
        ...
        model = JointOtherShareModel(target, epsilon=epsilon, penalty=penalty).fit(train)
    ...
    mass = rows.groupby(GROUP)[["joint_other", "predicted_other_mass", "actual_share"]].agg(
        {"joint_other": "sum", "predicted_other_mass": "first", "actual_share": "sum"})
    if not np.allclose(mass.joint_other + mass.predicted_other_mass, 1.0, atol=1e-10):
    ...
    for control in ("rolling3", "normalized_prior"):
        ...
        paired["delta_abs_error"] = (np.abs(rows.joint_other - rows.actual_share)
                                     - np.abs(rows[control] - rows.actual_share))
        meta["comparisons"][f"joint_other_minus_{control}"] = paired_week_interval(paired, n_bootstrap, seed)
    meta.update(status="complete", model="joint_softmax_with_explicit_other_bucket",
                epsilon=epsilon, penalty=penalty,
                represented_team_mass={"actual_mean": float(mass.actual_share.mean()),
                                       "candidate_mean": float(mass.joint_other.mean()),
                                       "candidate_mass_mae": float(np.abs(mass.joint_other - mass.actual_share).mean()),
                                       "other_mean": float(mass.predicted_other_mass.mean())},
                limitations=["Capped roster evaluation; no full-population or PPR result.",
                             "2023-2025 seasons were inspected in the previous mixed-effects run, so this is iterative confirmation.",
                             "The fractional softmax is a joint share model; it does not estimate team volume or served fantasy points."])
    return rows, meta
