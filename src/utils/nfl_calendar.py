...
def is_draft_prep_window(today: Optional[datetime] = None) -> bool:
    """
    True during the summer draft preparation window (June–August).

    During this period the upcoming NFL season hasn't started, but fantasy
    drafts happen before Week 1.  The training pipeline should:
      - train on all completed historical seasons (including the most recent)
      - designate the upcoming calendar year as the projection target season
      - allow an empty test set (no actuals exist yet for the future season)
    """
    t = today if today is not None else datetime.now()
    return t.month in (6, 7, 8)


def get_projection_season(today: Optional[datetime] = None) -> int:
    """
    The NFL season we are currently generating projections *for*.

    During the draft prep window (June–Aug) this is the upcoming calendar
    year (e.g. 2026 in August 2026).  Otherwise it matches the current
    NFL season returned by get_current_nfl_season().
    """
    t = today if today is not None else datetime.now()
    if is_draft_prep_window(t):
        return t.year
    return get_current_nfl_season(t)
